...
export default function DashboardPage() {
  ...
  const handleQuery = async (e: React.FormEvent) => {
    ...
    setQuery(""); // clear textarea immediately
    setLoading(true);
    setCurrentAnswer("");
    setCitations([]);
  ...
  return (
    <>
      <style>{`
        ...
        .query-bubble { margin-bottom: .75rem; display: flex; justify-content: flex-end; }
        .query-text {
          max-width: 75%;
          ...
          padding: .75rem 1rem;
          font-size: .88rem; color: #f5f0e8; line-height: 1.5;
        }

        .answer-bubble { margin-bottom: .5rem; }
        .answer-label {
        ...
        .query-textarea {
          ...
          min-height: 48px !important; max-height: 120px !important;
        ...
        .send-btn svg { color: #0a0a0b; width: 16px; height: 16px; }

        .input-hint {
          font-size: .72rem; color: rgba(245,240,232,.18);
          margin-top: .5rem; font-weight: 300;
          display: none;
        }
        @media (min-width: 640px) { .input-hint { display: block; } }
        ...
        .citations-badge {
          background: rgba(196,152,58,.2); color: #c4983a;
          font-size: .65rem; font-weight: 600;
          padding: 1px 6px; border-radius: 10px;
        }
        .citations-chevron {
          color: rgba(245,240,232,.25); width: 15px; height: 15px;
        ...
        .empty-citations {
          display: flex; flex-direction: column;
          align-items: center; justify-content: center;
          height: 200px; gap: 8px; opacity: .3; text-align: center;
        }
        .empty-citations svg { color: rgba(245,240,232,.4); width: 28px; height: 28px; }
        .empty-citations p { font-size: .78rem; color: rgba(245,240,232,.7); font-weight: 300; }
      `}</style>
        ...
        <nav className="dash-nav">
          ...
          <div className="nav-right">
            <div className="nav-links">
              <a href="/dashboard" className="nav-link-btn active">
                Chat
              </a>
              <a href="/sources" className="nav-link-btn">
                Sources
              </a>
            ...
            {messages.length > 0 && (
              <button
                ...
                onMouseOver={(e) => {
                  e.currentTarget.style.color = "rgba(245,240,232,.8)";
                  e.currentTarget.style.background = "rgba(255,255,255,.06)";
                }}
                onMouseOut={(e) => {
                  e.currentTarget.style.color = "rgba(245,240,232,.4)";
                ...
                <div key={i}>
                  ...
                  {msg.role === "user" ? (
                    <div className="query-bubble">
                      <div className="query-text">{msg.content}</div>
                    </div>
                  ) : (
                    <div className="answer-bubble">
                      <div className="answer-label">ContextIQ</div>
                      {msg.citations && msg.citations.length > 0 && (
                        <div
                          style={{
                            fontSize: ".7rem",
                            color: "rgba(196,152,58,.6)",
                            marginBottom: ".5rem",
                            display: "flex",
                            alignItems: "center",
                            gap: "4px",
                        ...
                        >
                          ...
                          Confidence:{" "}
                          {Math.round(
                            (msg.citations.reduce(
                              (sum: number, c: any) => sum + c.similarity,
                              0,
                            ) /
